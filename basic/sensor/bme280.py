...
class Bme280():
    ...
    def __init__(self, bus_number=1, dev_address=0x76):
    ...
    def read_data(self):
        ...
        press_raw = (self.data[0] << 12) | (self.data[1] << 4) | (self.data[2] >> 4)
        temp_raw = (self.data[3] << 12) | (self.data[4] << 4) | (self.data[5] >> 4)
        humid_raw  = (self.data[6] << 8)  |  self.data[7]
        temp = self._get_temp(temp_raw)
        press = self._get_pressure(press_raw)
        humid = self._get_humid(humid_raw)
        print(press_raw, temp_raw, humid_raw)
        return temp, humid, press

    def _get_pressure(self, press_raw):
        pressure = 0.0
        ...
        if v1 == 0:
            return 0
        pressure = ((1048576 - press_raw) - (v2 / 4096)) * 3125
    ...
    def _get_humid(self, humid_raw):
        var_h =   self.t_fine  - 76800.0
        if var_h != 0:
            var_h = (humid_raw - (self.digH[3] * 64.0 + self.digH[4]/16384.0 * var_h)) * (self.digH[1] / 65536.0 * (1.0 + self.digH[5] / 67108864.0 * var_h * (1.0 + self.digH[2] / 67108864.0 * var_h)))
        else:
            return 0
...
def main():
    ...
    temp, humid, press = bme280.read_data()
    print("Temp:", temp, "C")
    print("Humid:", humid, "%")
